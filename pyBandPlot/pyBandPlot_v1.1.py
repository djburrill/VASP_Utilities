...
v1.1
- Added ability to only plot DOS
...
if __name__ == '__main__':
    ...
    if (dosInc == True):
        ...
        # If the bands are also included
        if (bandInc == True):
            dosGraph = pylab.subplot(1,2,2)
        
        # Plot if not Header
        for index,row in enumerate(dosData[1:]):
            pylab.plot(row,dosData[0],antialiased=True,
                       label=paramDict['doslab'][index+1],
                       lw=1.5,c=paramDict['doscol'][index])
